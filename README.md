...
**Status:** pre-alpha. Architecture spec landed; DSL/IR Phase 1, Provisioner Phases 1–2, Inventory Persistence Phase 1, AWS Expansion Phase 3, Parity Engine Phase 1, Cost Estimator Phase 1, Azure Adapter Phase 4, GCP Adapter Phase 5, Validator Phase 4 (per-type spec schemas), Validator Phase 5 (cross-component refs), Secrets Phase 1 (env-var + file credential backends), and Web App UI Phase 1 (read-only browser pages at `/ui/projects`, `/ui/deployments/{id}`, `/ui/runs/{id}`), and Web App HTTP Phase 1 (JSON GET endpoints at `/api/v1/projects`, `/api/v1/deployments/{id}`, `/api/v1/runs/{id}` with optional `NIMBUSFAB_API_TOKEN` Bearer auth), and Web App HTTP Phase 2 (browser-triggered deployments: `POST /api/v1/deployments/{id}/applies` returns 202 async; `GET /api/v1/deployments/{id}/events` streams RunEvents live via SSE), and Web App UI Phase 2 (Deploy/Destroy/Drift buttons + live log pane; vanilla JS + EventSource, no framework), and Inventory Phase 2 (Postgres backend; `NIMBUSFAB_DB_DSN=postgres://...` for multi-instance web-app deployment, SQLite remains the local-dev default) merged. `credentialRef: aws-dev` now resolves through `NIMBUSFAB_SECRET_AWS_DEV` env var (JSON) or `~/.nimbusfab/secrets/aws-dev.json`; the resolved keys flow into the Tofu runner's per-command env so cloud providers authenticate. Three-cloud projects work end-to-end: a project targeting `[aws, azure, gcp]` produces per-cloud Tofu workspaces, the parity engine reports real cross-cloud SKU divergence with 3-way weighted scores, and the cost estimator shows per-cloud subtotals.
