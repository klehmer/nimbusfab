...
func (r *apiTokenRepo) GetByPrefix(ctx context.Context, prefix string) (*inventory.ApiToken, error) {
    ...
    `, prefix).Scan(&t.ID, &t.OrgID, &t.UserID, &hash, &t.Name, &t.Prefix, &createdAt, &lastUsedAt)
	...
	if lastUsedAt.Valid {
		lt := mustParseTime(lastUsedAt.String)
		t.LastUsedAt = &lt
	}
	return &t, nil
}
...
func (r *apiTokenRepo) ListByUser(ctx context.Context, orgID, userID string) ([]inventory.ApiToken, error) {
	rows, err := r.db.QueryContext(ctx, `
        SELECT id, org_id, user_id, token_hash, name, prefix, created_at, last_used_at
        FROM api_tokens WHERE org_id = ? AND user_id = ?
	...
	for rows.Next() {
		...
		var lastUsedAt sql.NullString
		var createdAt string
		if err := rows.Scan(&t.ID, &t.OrgID, &t.UserID, &hash, &t.Name, &t.Prefix, &createdAt, &lastUsedAt); err != nil {
			return nil, err
		}
		t.TokenHash = hash
		t.CreatedAt = mustParseTime(createdAt)
...
func (r *apiTokenRepo) Revoke(ctx context.Context, orgID, id string) error {
	_, err := r.db.ExecContext(ctx,
		"DELETE FROM api_tokens WHERE org_id = ? AND id = ?",
		orgID, id)
	if err != nil {
