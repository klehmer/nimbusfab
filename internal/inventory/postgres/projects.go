...
func (r *projectRepo) Get(ctx context.Context, orgID, id string) (*inventory.Project, error) {
	...
	err := r.db.QueryRowContext(ctx,
		"SELECT id, org_id, name, COALESCE(source_uri, ''), created_at FROM projects WHERE org_id = $1 AND id = $2",
		orgID, id).Scan(&p.ID, &p.OrgID, &p.Name, &p.SourceURI, &p.CreatedAt)
...
func (r *projectRepo) List(ctx context.Context, orgID string) ([]inventory.Project, error) {
	rows, err := r.db.QueryContext(ctx,
		"SELECT id, org_id, name, COALESCE(source_uri, ''), created_at FROM projects WHERE org_id = $1 ORDER BY name",
		orgID)
	if err != nil {
	...
	for rows.Next() {
		var p inventory.Project
		if err := rows.Scan(&p.ID, &p.OrgID, &p.Name, &p.SourceURI, &p.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, p)
	}
...
func (r *projectRepo) Create(ctx context.Context, p inventory.Project) error {
	_, err := r.db.ExecContext(ctx,
		"INSERT INTO projects (id, org_id, name, source_uri) VALUES ($1, $2, $3, $4)",
