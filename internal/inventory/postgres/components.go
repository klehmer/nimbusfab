...
func (r *componentRepo) Get(ctx context.Context, orgID, id string) (*inventory.Component, error) {
	...
	err := r.db.QueryRowContext(ctx,
		"SELECT id, org_id, project_id, stack_id, name, type, COALESCE(ir_json::text,''), updated_at FROM components WHERE org_id = $1 AND id = $2",
		orgID, id).Scan(&c.ID, &c.OrgID, &c.ProjectID, &c.StackID, &c.Name, &c.Type, &irJSON, &c.UpdatedAt)
...
func (r *componentRepo) ListByStack(ctx context.Context, orgID, projectID, stackID string) ([]inventory.Component, error) {
	rows, err := r.db.QueryContext(ctx,
		"SELECT id, org_id, project_id, stack_id, name, type, COALESCE(ir_json::text,''), updated_at FROM components WHERE org_id = $1 AND project_id = $2 AND stack_id = $3 ORDER BY name",
	...
	for rows.Next() {
		var c inventory.Component
		var irJSON string
		if err := rows.Scan(&c.ID, &c.OrgID, &c.ProjectID, &c.StackID, &c.Name, &c.Type, &irJSON, &c.UpdatedAt); err != nil {
			return nil, err
		}
		c.IRJSON = []byte(irJSON)
		out = append(out, c)
...
func (r *componentRepo) Upsert(ctx context.Context, c inventory.Component) error {
	_, err := r.db.ExecContext(ctx, `
        ...
        ON CONFLICT (project_id, stack_id, name) DO UPDATE SET
            ...
            updated_at = now()
    `, c.ID, c.OrgID, c.ProjectID, c.StackID, c.Name, c.Type, jsonOrEmpty(c.IRJSON))
	if err != nil {
